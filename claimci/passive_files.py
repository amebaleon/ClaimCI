...
class PassiveFileInspection:
    """One streaming size/hash inspection with no retained file content."""
...
def inspect_confined_regular_file(
    ...
    *,
    ...
    """Hash one stable confined regular file without retaining its bytes."""
    ...
    try:
        ...
        if _file_open_identity(opened_after) != _file_open_identity(opened):
            raise PassiveFileError(
                "passive file descriptor changed during inspection",
                code="changed",
            )
    except PassiveFileError:
        raise
    ...
    try:
        ...
        if _path_identity(root_after) != _path_identity(root_before):
            raise PassiveFileError(
                "passive file root identity changed during inspection",
                code="changed",
            )
        for component, identity in component_identities[:-1]:
            if _path_identity(os.lstat(component)) != identity:
                raise PassiveFileError(
                    "passive file parent identity changed during inspection",
                    code="changed",
                )
        after = os.lstat(current)
    except PassiveFileError:
    ...
    if _file_path_identity(after) != _file_path_identity(before):
        raise PassiveFileError(
            "passive file pathname changed during inspection",
            code="changed",
        )
    if _file_open_identity(after) != _file_open_identity(opened_after):
        raise PassiveFileError(
            "passive file identity changed during inspection",
            code="changed",
        )
